...
public class StepCafeManha : FlowBase
{
    public override bool IsMatch(DataExchangeRequest model)
    {
        return model.Screen == "STEP_CAFE_MANHA" && model.Action == "data_exchange";
    }
    ...
    private async Task<DataExchangeResponse> FinalizarAsync(DataExchangeRequest model, IFlowRepository flowRepository, IKeyValueRepository keyValueRepository)
    {
        ...
        try
        {
            ...
            data.CafeManha = new RefeicaoModel
            {
                TipoProdutoEnum = TipoProdutoEnum.CafeManha,
        ...
        switch (tipoConsumo)
        {
            case TipoConsumoEnum.Retirada:
                restaurantes = restaurantes
                    .Where(r => r.CafeManhaAceitaRetirada)
                    .ToList();
                break;

            case TipoConsumoEnum.Local:
                restaurantes = restaurantes
                    .Where(r => r.CafeManhaAceitaConsumoLocal)
                    .ToList();
                break;
